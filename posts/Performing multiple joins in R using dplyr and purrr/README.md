...
The reduce operation is a technique that combines all the elements of a vector (i.e. an array containing our datasets) into a single value (i.e. the final joined table) by iteratively applying a function that takes two arguments and chaining the output of one iteration into the input of the next
